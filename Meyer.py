...
def mse(dataset, reg1, reg2):
    ...
    for i in range(len(dataset)):
        msev = msev + (dataset[i][1] - (reg1 * math.sqrt(dataset[i][0])) - reg2) ** 2
    return msev / len(dataset)

def msedw(dataset, reg1, reg2):
    msedv = 0
    for i in range(len(dataset)):
        msedv = msedv + 2 * (reg1 * (math.sqrt(dataset[i][0]) ** 2) - (dataset[i][1] * math.sqrt(dataset[i][0])) + reg2 * math.sqrt(dataset[i][0]))
    return msedv / len(dataset)

def msedb(dataset, reg1, reg2):
    msedv = 0
    for i in range(len(dataset)):
        msedv = msedv + 2 * (reg2 - dataset[i][1] + (reg1 * math.sqrt(dataset[i][0])))
    return msedv / len(dataset)
...
for i in range(1, 10):
    plt.scatter(i, (w * math.sqrt(i)) + b, s = 10, c = 'r')
    if i == 9:
        plt.text(i, (w * math.sqrt(i)) + b, "MSE: "+str(abs(mse(train, w, b))))
